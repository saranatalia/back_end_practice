...
puts "What is your date of birth? Please write the digits in the form MMDDYYYY with no punctuation in between."
birthdate = gets.to_s
number = determine_path_number(birthdate)
message = determine_message(number)

puts "Your numerology number is #{number}. #{message}"

# **WHEN YOU CALL METHOD ALWAYS INCLUDE ARGUMENT!**
# **ALSO WRITE COMMANDS AT THE BOTTOM, AFTER METHOD, THIS IS WHERE YOU CALL THE METHOD**
# **WORK ON THIS: **
# birthdate = gets.to_s
# number = determine_path_number(birthdate)
# message = determine_message(number)
